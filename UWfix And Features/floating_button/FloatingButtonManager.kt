...
object FloatingButtonManager {
    ...
    fun update(context: Context) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            mainHandler.post { update(context) }
            return
        }
        ...
        val permissionGranted = hasOverlayPermission(appContext)
    ...
    private fun launchMainActivity(context: Context) {
        try {
            ...
            AppLog.e("FloatingButtonManager: Failed to launch MainActivity", e)
        }
    }

    fun removeOverlay(context: Context) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            mainHandler.post { removeOverlay(context) }
            return
        }
